...
import { AVAILABLE_STATUS, BOOKING_STATUS, USER_ROLES } from "../config/constants";

export class BookingService {
    async createBooking(bookingData: IBooking): Promise<IBookingResponse> {
        ...
        const customerResult = await pool.query(`SELECT id FROM users WHERE id = $1`, [customer_id]);
        ...
        const vehicleResult = await pool.query(`SELECT id, vehicle_name, daily_rent_price, availability_status FROM vehicles WHERE id = $1`, [vehicle_id]);
        ...
        if(vehicle.availability_status !== AVAILABLE_STATUS.AVAILABLE){
        ...
        try{
            ...
            await client.query(`UPDATE vehicles SET availability_status = $1 WHERE id = $2`, [AVAILABLE_STATUS.BOOKED, vehicle_id]);
            ...
            const bookingResult = await client.query(
                `INSERT INTO bookings (customer_id, vehicle_id, rent_start_date, rent_end_date, total_price, status) 
                 VALUES($1, $2, $3, $4, $5, $6) 
                 RETURNING *`,
                [customer_id, vehicle_id, rent_start_date, rent_end_date, totalPrice, BOOKING_STATUS.ACTIVE]
            );
    ...
    async getAllBooking(
        ...
        return bookingResult.rows.map((row: any) => {
            if(currentUserRole === USER_ROLES.ADMIN){
                return {
                    id: row.id,
    ...
    async updateBooking(
        ...
        if(currentUserRole !== USER_ROLES.ADMIN && booking.customer_id !== currentUserId){
            const error = new Error('Forbidden: You can only updte your own bookings');
            (error as any).statusCode = 404;
            throw error;
        }

        const validStatus = [BOOKING_STATUS.CANCELLED, BOOKING_STATUS.RETURNED] as const;
        ...
        if(currentUserRole === USER_ROLES.CUSTOMER && status === BOOKING_STATUS.CANCELLED){
        ...
        if(booking.status === BOOKING_STATUS.CANCELLED || booking.status === BOOKING_STATUS.RETURNED ){
        ...
        try{
            ...
            if(status === BOOKING_STATUS.RETURNED){
                await client.query(`
                    UPDATE vehicle SET available_status = $1 WHERE id = $2
                    `, [AVAILABLE_STATUS.AVAILABLE, booking.vehicle_id]);
            }

            // jodi status 'cancelled' hoi, update vehicle status = 'availbale'
            if(status === BOOKING_STATUS.CANCELLED){
                await client.query(`UPDATE vehicles SET availability_status = $1 WHERE id = $2`, [AVAILABLE_STATUS.AVAILABLE, booking.vehicle_id]);
            }
            ...
            if(status === BOOKING_STATUS.RETURNED){
                const vehicleResult = await pool.query(
                    `SELECT available_status FROM vehicles WHERE id = $1`,
                    [booking.vehicle_id]
                );
